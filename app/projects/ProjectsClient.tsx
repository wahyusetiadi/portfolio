...
export default function ProjectsPage({ initialData }: { initialData: PortfolioData }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      {/* Navbar */}
      <nav aria-label={lang === 'id' ? 'Navigasi proyek' : 'Project navigation'} className="sticky top-0 z-[100] border-b border-[var(--border)] bg-[var(--bg-surface)]">
      ...
      <div className="mx-auto max-w-[1200px] px-7 pt-[60px]">
        ...
        {/* Filters */}
        <div className="mb-12 flex flex-wrap items-start gap-5 border-b border-[var(--border)] pb-7">
          {/* Status filter */}
          {publicOngoing.length > 0 && <div className="flex flex-wrap gap-1.5">
          ...
          {/* Tag filter */}
          {allTags.length > 0 && <details className="project-filter-details">
            <summary>{lang === 'id' ? 'Filter teknologi' : 'Filter technology'}{filterTag ? `: ${filterTag}` : ''}</summary>
      ...
      <div className="mx-auto max-w-[1200px] px-7 pb-20">
        ...
        {(filterStatus === 'all' || filterStatus === 'in-progress' || filterStatus === 'review' || filterStatus === 'planning') && filteredOngoing.length > 0 && (
          <div className="mb-[72px]">
            <div className="mb-7 flex items-center gap-3.5">
              <h2 className="mb-0 block font-mono text-[11px] tracking-[0.12em] text-[var(--accent)]">{tx(UI.ongoing.label, lang)}</h2>
        ...
        {(filterStatus === 'all' || filterStatus === 'completed') && (
          <div>
            <div className="mb-7 flex items-center gap-3.5">
              <h2 className="mb-0 block font-mono text-[11px] tracking-[0.12em] text-[var(--accent)]">{tx(UI.projectsPage.completed, lang)}</h2>
              <span className="font-mono text-[11px] text-[var(--text-muted)]">
                  ...
                  <article key={p.id} className="project-item">
                    ...
                    <div className="project-body">
                      ...
                      <div className="project-links">
                        <Link href={`/projects/${p.id}`}>{lang === 'id' ? 'Lihat detail' : 'View details'} ↗</Link>
                        {p.github && <a href={p.github} target="_blank" rel="noopener noreferrer">{tx(UI.projects.github, lang)} ↗</a>}
                        {p.demo && <a href={p.demo} target="_blank" rel="noopener noreferrer">{tx(UI.projects.demo, lang)} ↗</a>}
                      </div>
                    </div>
                  </article>
                ))}
